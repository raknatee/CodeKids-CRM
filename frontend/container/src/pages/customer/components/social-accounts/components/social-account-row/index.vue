<template>
  <div style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 0.75rem; align-items: center">
    ...
    <input
      ...
      placeholder="Platform_user_id"
      :style="inputStyle"
      @input="$emit('update:modelValue', { ...modelValue, userId: ($event.target as HTMLInputElement).value })"
    />
    <button
      type="button"
...
function handlePlatformChange(value: string): void {
  emit("update:modelValue", { ...props.modelValue, platform: value as SocialPlatform });
}
...
const removeButtonStyle = {
  ...
  width: "2.25rem",
  height: "2.25rem",
  backgroundColor: isRemoveHover.value ? "#fee2e2" : "#ffffff",
  cursor: "pointer",
  color: isRemoveHover.value ? "#dc2626" : "#6b7280",
